...
@app.command()
def list_skills(target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")):
    """列出可用的技能"""
    mgr = SkillManager(target)
    mgr.list_available()

@app.command()
def installed(target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")):
    """列出已安装的技能"""
    mgr = SkillManager(target)
    mgr.list_installed()
...
def install(
    ...
    target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")
):
    """安装指定的技能"""
    mgr = SkillManager(target)
    for skill in skills:
...
@app.command()
def install_all(target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")):
    """安装所有技能"""
    mgr = SkillManager(target)
    mgr.install_all()

@app.command()
def install_commands(target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")):
    """安装命令"""
    mgr = SkillManager(target)
    mgr.install_commands()

@app.command()
def interactive(target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")):
    """交互式安装"""
    mgr = SkillManager(target)
    mgr.interactive()

@app.command()
def prompt_update(target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")):
    """更新 CLAUDE.md 提示文件"""
    mgr = SkillManager(target)
    mgr.prompt_update()

@app.command()
def prompt_diff(target: str = typer.Option("claude", "--target", "-t", help="Target platform (claude, codex, gemini)")):
